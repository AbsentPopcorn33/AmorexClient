        ...
        this.chatInput.addEventListener("keydown", (event) => {
            ...
            if (event.keyCode === global.KEY_ENTER) {
                ...
                if (message.startsWith("/render")) {
                    ...
                    if (args[2]) { try { color = gameDraw.getColor(args[2]) || color } catch (e) { console.log(e) } }
                    let tanks = new Array(), tankNameList = new Array();
                    if (args[1]) {
                        if (args[1] === "all") { for (let mockup of global.mockups) { tanks.push(mockup.index), tankNameList.push(mockup.className) }; console.log(tankNameList); } else
                            for (let tankName of args[1].split("-")) { let mockup = global.mockups.find((i) => i.className == tankName || i.name == tankName)?.index; mockup ? tanks.push(mockup) : console.log(`mockup for ${tankName} Failed`) };
                    }
                    let tankData = tanks.length ? util.getEntityImageFromMockup(tanks.join("-"), color) : false;
                    if (args[1] === "all") console.log(tankData);

                    if (tankData) {
                        let tankDataImg = document.createElement('canvas'),
                            canvasSize = 1000,
                        ...
                        try {
                            ...
                            if (ImageData) navigator.clipboard.write([
                                ...
                                global.createMessage("Render Copyed to Clipboard", 10000);
                            else global.createMessage("Render Failed", 10000);
                        } catch (error) { global.createMessage("Render Failed Error in Console", 10000); console.error(error); }
                        tankDataImg.remove()
                    } else global.createMessage("Tank Not Found", 10000);
                } else
                    //Render Command End
